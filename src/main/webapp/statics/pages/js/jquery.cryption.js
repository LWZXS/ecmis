...
(function($){
	...
	jQuery.aes = {
			 ...
			 *  根据种子进行加密
			 * @method encrypt
			 * @param {object} key 必需，种子数据信息。
			 * @param {object} msg 必需，有效数据信息。
			 * @return {string}  加密字符串
			 * @example
			 *      var value = $.aes.encrypt(‘12344’,'123456');
			 */
			 ...
			 * @return {string}  解密字符串
			 * @example
			 *      var value = $.aes.decrypt(‘12344’,'123456');
			 */
...
/**
 * <b>Superclass</b> description.<br>
 * 星盾平台安全组件-base64加解密工具类
 * 
 * @module com.aostarit.framework.rwc
 * @submodule com.aostarit.framework.base
 * @namespace com.aostarit.framework.base.cryption
 * @main com.aostarit.framework.rwc
 * @constructor
 * @class base64
 * @since 3.0
 */
(function($){
	jQuery.base64 = {
			 ...
			 * @return {string}  加密字符串
			 * @example
			 *      var hashcode = $.base64.encrypt('123456');
			 */
...
(function($) {
	...
	$.extend({
		 ...
		 *  应用md5算法生成hash值
		 * @method md5
		 * @param {object} string 必需，有效数据信息。
		 * @return {json}  json对象
		 * @example
		 * 		//获取模与公钥指数
		 *      var hashcode = $.md5('123456');
		 */
		md5 : function(string) {
			var x = Array();
 ...
 * document.forms[0].password.value = RSAUtils.encryptedString(key, pwd);
 */
/**
 * <b>Superclass</b> description.<br>
 * 星盾平台安全组件-rsa加解密工具类
...
/**
 * 封装rsautils的方法，包装原有接口，便于按照统一的思想编程
 */
(function($) {
	/**
	 * 应用jquery的类继承函数进行对象集成
	...
	jQuery.rsa = {
	     ...
	     *      jq:  必需。 一个jq对象
		 * @method getKeyPair
		 * @param {object} encryptionExponent 必需。 加密公钥指数。
		 * @param {object} decryptionExponent 必需。 解密私钥指数。
		 * @param {object} modulus 必需。 模。
		 * @return {json}  json对象
		 * @example 
		 *      //exponent、module通过远程调用获取，并通过前台rsa的获取密钥对方法生成解密key。
		 *      var key=$.rsa.getKeyPair(exponent,"",module)
		 */
		getKeyPair : function(encryptionExponent, decryptionExponent, modulus) {
			return RSAUtils.getKeyPair(encryptionExponent, decryptionExponent,
					modulus);
		},
		/**
		 *  根据种子进行加密
		 * @method encrypt
		 * @param {object} key 必需。 秘钥对。
		 * @param {object} msg 必需。 编码信息。
		 * @return {string}  加密字符串
		 * @example
		 *      //exponent、module通过远程调用获取，并通过前台rsa的获取密钥对方法生成解密key。
		 * 		//获取模与公钥指数
		 ...
		 *      var msg='123456';
		 *      //进行加密
		 *      var value=$.rsa.encrypt(key, msg);
		 */
		encrypt : function(key, msg) {
			return RSAUtils.encryptedString(key, msg);
		},
		/**
		 *  根据种子进行解密
		 * @method decrypt
		 ...
		 */
		decrypt : function(key, msg) {
			return RSAUtils.decryptedString(key, msg);
		}
	};
})(jQuery);
