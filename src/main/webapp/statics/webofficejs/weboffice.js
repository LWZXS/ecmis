...
function openfile(){ 
	try{
		switch(flag){
			case '11':
				frm.WebOffice.Open(strOpenUrl,true,"Word.Document","","");
				break;
			case '12':
				frm.WebOffice.Open(strOpenUrl,true,"Excel.Sheet","","");
				break;
			case '13': 
				frm.WebOffice.Open(strOpenUrl,true,"PowerPoint.Show","","");
				break;
			case '2':
				frm.WebOffice.CreateNew("Excel.Sheet.8");	
				break;
			case '3':
...
function WebHttpSave(){
	try	{
		...
		//frm.WebOffice.HttpAddPostFile('file1',frm.file1.value);
		frm.WebOffice.HttpAddPostCurrFile("docfile","");	
//		frm.WebOffice.HttpPost(strRoot + '/fileadddocupload.jsp');
		frm.WebOffice.HttpPost(frm.url.value);
		//frm.WebOffice.HttpPost('http://111.230.236.94/ecmis/upload.jsp');
		//frm.WebOffice.HttpPost('http://ehhqu488106.jsp.jspee.org/ecmis/upload.jsp');
		alert("保存成功！");
	}catch(e){
		alert('保存Office文档发生错误！');
	}
...
function WebSavePPTLocalJPG(){
	try{
		...
		// frm.WebOffice.ActiveDocument.Application.ActivePresentation.SaveAs
		// ("c:\\a15",6);//rtf
		// frm.WebOffice.ActiveDocument.Application.ActivePresentation.SaveAs
		// ("c:\\a15",7);//pps
		// frm.WebOffice.ActiveDocument.Application.ActivePresentation.SaveAs
...
function WebSignature(str){
	...
	// doc.Shapes.AddPicture(strPic, false,
	// true,100,0,207,209,doc.Application.Selection.Range);
...
function WebDocSignature(){
	try{
		...
		// doc.Shapes.AddPicture('c:\\Sign.bmp', false,
		// true,100,0,219,112,doc.Application.Selection.Range);
		doc.Shapes(doc.Shapes.Count).Select(); 
		var range = doc.Application.Selection.ShapeRange;
		range.WrapFormat.Type = 3;
		range.PictureFormat.TransparentBackground = true;
...
/*
 * for (var n=0; n<iColumns; n++) { for (var i=0; i<iCells; i++) { iPos =
 * mText.indexOf(";",1+iold); mTmp = mText.substring(iold+1,iPos);
 * frm.WebOffice.ActiveDocument.Tables(1).Columns(n+1).Cells(i+1).Range.Text=mTmp;
 * //填充单元值 iold = iPos; } }
 */
	}catch(e){
		alert(e);
