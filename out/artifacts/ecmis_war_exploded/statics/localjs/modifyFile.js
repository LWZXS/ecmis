...
function changeMainBodyInput(){
	//改mainBody的值
	window.parent.document.getElementById("mainBody").value=window.parent.document.getElementById("newMainBody").value
	//alert(window.parent.document.getElementById("newMainBody").value);
}

function save(status){
	
	//改mainBody的值
	//$("#mainBody").val($("#newMainBody").val());
	...
	$("#status").val(status);
	 $('#ff').form('submit', {
	    ...
	    onSubmit: function(){
			...
			*/
			var flag= check($('[name="address"]')) & 
				check($('[name="docSource"]')) & 
				check($('#docTypeName')) & 
				/*check($('[name="serviceNo"]')) & */
				check($('[name="documentName"]')) & 
				check($('#subjectTermName')) & 
				check($('#auditorName')) & 
				check($('[name="originalCode"]')) & 
				check($('#approverName')) & 
				/*check($('[name="creationDate"]')) &*/
				/*check($('#joint_trial_name')) & */
				/*check($('#publish_range_name')) & */
				check($('#docTypeName')) & 
				check($('#subjectTermName')) & 
				check($('#auditorName')) & 
				check($('#approverName'));
...
function getSelectedType(source,targetId,targetText,dialogObj){
	...
	if (node){
		...
		//向服务器发送一个请求
		$.ajax({
			url:'documentType/putModifyWordPath',
			type:'get',
			data:'docName='+node.attributes.docName+'&documentId='+$("#documentId").val()+'&mainBody='+$("#mainBody").val()+'&docTypeId='+$("#currentDocTypeId").val()+'&selectDocTypeId='+node.id,
			dataType:'json',
			success:function(data){
				if(data.result=='success'){
					...
					if($('#file-tabs').tabs('exists','正文')){
						...
						if($("#newMainBody").val()==""){
							...
							$("#newMainBody").val(mainBody+".doc");
							$("#newMainBodyNum").val(mainBody);
						}else{
							mainBody=$("#newMainBodyNum").val();
							
						}
						
						$('#file-tabs').tabs('add',{
							title: '正文',
							content: '<iframe src="document/selectModifyWord?oper=new&flag=1&mainBody='+mainBody+'&currentDocTypeId='+node.id+'+&docName='+node.attributes.docName+'" style="width:100%; height:900px"></iframe>',
...
//alert($("#searchType").val().trim());
	$("#typeTree").tree({
		url : "documentType/findAll?param="+$("#searchType").val().trim(),//请求路径，id为根节点的id  
		onLoadSuccess : function(node, data) {
			var tree = $(this);
			if (data) {
				$(data).each(function(index, d) {
...
$(function(){
	...
	$("#typeBtnLoad").click(function(){
		...
		loadTree($("#typeTree"),"documentType/findAll?param="+$("#searchType").val().trim());
	});
	//主题词
