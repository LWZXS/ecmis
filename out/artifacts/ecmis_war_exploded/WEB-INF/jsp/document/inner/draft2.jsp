...
<html>
  ...
  <body>
	...
	<div id="tb" style="padding:2px 5px;">
		<form action="<%=request.getContextPath()%>/document/pending.json" id="pendingFrm">
	...
	<script type="text/javascript">
		$(function(){
			...
			$("#searchBtn").click(function(){
				...
				$('#dg').datagrid({
					url:'<%=request.getContextPath()%>/document/pending.json?'+$("#pendingFrm").serialize()
			...
			//加载数据
			$('#dg').datagrid({
				url:'<%=request.getContextPath()%>/document/pending.json',
				queryParams: {
			    ...
			    onDblClickRow:function(rowIndex,rowData){
			    	window.location.href='<%=request.getContextPath()%>/document/view/'+rowData.documentId+'/draft';
			    },
				columns:[[
